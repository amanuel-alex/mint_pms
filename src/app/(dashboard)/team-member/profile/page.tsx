  ...
  const onSubmitProfile = async (data: z.infer<typeof profileSchema>) => {
    ...
    try {
      if (selectedFile) {
        ...
        setProfile((prev: any) => ({ ...prev, profileImageUrl: imageUrl }));
        ...
        {/* Header */}
            <div className="w-full flex flex-col items-center gap-2 mb-2">
              <h1 className="text-4xl font-extrabold tracking-tight text-[#087684] flex items-center gap-3">
                ...
                <span className="bg-[#FB923C] text-white px-4 py-1 rounded-full text-sm font-semibold shadow">{profile?.role?.replace('_', ' ')}</span>
              </h1>
            ...
            <form onSubmit={handleProfileSubmit(onSubmitProfile)} className="w-full flex flex-col md:flex-row gap-8 items-center md:items-start">
              <div className="flex flex-col items-center gap-3 w-full md:w-1/3">
                <div className="relative">
                  <Avatar className="w-32 h-32 ring-4 ring-[#FB923C] ring-offset-2 shadow-lg">
                    <AvatarImage src={preview ? preview : (profile?.profileImageUrl ? profile.profileImageUrl : undefined)} />
            ...
            <div>
              ...
              {profileErrors.email && (
                ...
                <div>
                  ...
                  <div className="font-semibold text-[#087684] mt-1">{profile?.role?.replace('_', ' ')}</div>
                </div>
                <div>
                ...
                <div className="col-span-2 min-w-0 flex flex-col sm:flex-row items-stretch sm:items-center gap-3 mt-4">
                  ...
                  <Button type="button" variant="outline" className="w-full sm:flex-1 h-auto sm:h-11 py-2 text-sm sm:text-base max-w-full flex items-center justify-center whitespace-normal break-words" onClick={() => router.push('/team-member')} disabled={saving}>
                Cancel
              </Button>
                  <Dialog>
        ...
        {/* Password Change Form */}
          ...
          <form onSubmit={handlePasswordSubmit(onSubmitPassword)} className="space-y-6">
            ...
            <div className="flex gap-4">
                  ...
                  <Button type="button" variant="outline" className="flex-1 h-11 text-base" onClick={() => router.push('/team-member')} disabled={isSubmittingPassword}>
                Cancel
              </Button>
            </div>
          </form>
